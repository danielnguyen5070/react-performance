import { useState, use, useTransition, Suspense, memo, useRef } from 'react'
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-100 to-slate-300 p-6">
            <div className="w-full max-w-md bg-white shadow-lg rounded-2xl p-6 space-y-4">
                ...
                >
                    <li style={{ height: `${rowVirtualizer.getTotalSize()}px`, position: 'relative' }}>
...
const CityItem = memo(function (
    { city, index, isHighlighted, isSelected, getItemProps, start, size }:
        { city: CityProps, index: number, isHighlighted: boolean, isSelected: boolean, getItemProps: UseComboboxPropGetters<CityProps>['getItemProps'], start: number, size: number }) {
    return (
        <li
            key={city.id}
            {...getItemProps({
